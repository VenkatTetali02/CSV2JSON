...
def generate_json_from_csv(lst):
    ...
    query=f"""You are an expert reader in reading a array list of CSV strings.Dont provide any other supplemental information.
           Below are the few examples on how the information appears and how it should be formatted.
           Truncate any extra characters in the end

           Example1:
           {{"input":["012636633","Test Name1","Computer Science","3.7","Advisor Name1","\n","733939","Test Name2","Bachelors in History","3.2","Advisor Name2"]}}
            [
    ...
    return json_results[0]['outputText']

def lambda_handler(event, context):
    # TODO implement
    json_str=event['body']
    json_data=generate_json_from_csv(json_str)
    print('after stripping',json_data)
    json_final_data=json.loads(json_data)
    return {
        'statusCode': 200,
        'body': json.dumps(json_final_data)
    }
